...
class StatementCsvFormatterTest {
    ...
    fun `can format one-line statement`() {
        val formattedStatement = StatementCsvFormatter(Paths.get("${bankName}_test_one_line.csv"))
    ...
    fun `can format three-line statement`() {
        val formattedStatement = StatementCsvFormatter(Paths.get("${bankName}_test_three_lines.csv"))
        val expectedFormat =
                listOf(
    ...
    fun `can handle negative values`() {
        val formattedStatement = StatementCsvFormatter(Paths.get("${bankName}_test_three_lines_negative.csv"))
        val expectedFormat =
                listOf(
    ...
    fun `can handle trailing commas`() {
        val formattedStatement = StatementCsvFormatter(Paths.get("${bankName}_test_trailing_comma.csv"))
        val expectedFormat =
                listOf(
    ...
    fun `can format full statement`() {
        val formattedStatement = StatementCsvFormatter(Paths.get("${bankName}_test_full.csv"))
        val expectedFile = File(StatementCsvFormatter.baseInputPath.toString() + File.separator + "2017-10_Tom_${bankName.capitalize()}.csv")
